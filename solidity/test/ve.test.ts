...
describe('veBanny URI Resolver Tests', () => {
  ...
  const contributionTiers = [
    100, 200, 300, 400, 500, 600, 700, 800, 900, 1_000, 2_000, 3_000, 4_000, 5_000, 6_000, 7_000,
    8_000, 9_000, 10_000, 12_000, 14_000, 16_000, 18_000, 20_000, 22_000, 24_000, 26_000, 28_000,
    30_000, 40_000, 50_000, 60_000, 70_000, 80_000, 90_000, 100_000, 200_000, 300_000, 400_000,
    500_000, 600_000, 700_000, 800_000, 900_000, 1_000_000, 2_000_000, 3_000_000, 4_000_000,
    5_000_000, 6_000_000, 7_000_000, 8_000_000, 9_000_000, 10_000_000, 20_000_000, 40_000_000,
    60_000_000, 100_000_000, 600_000_000,
  ];
  ...
  before(async () => {
    ...
    uriResolver = await uriResolverFactory
      .connect(deployer)
      .deploy(
        storage.address,
  ...
  it('Character Definition Tests', async () => {
    ...
    const imageData = Buffer.from(
      ...
      json['image'].slice(json['image'].indexOf(',') + 1),
      'base64',
    ).toString();
  ...
  it('Contribution Tiers Tests', async () => {
    ...
    await uriResolver.tokenURI(
      1,
      contributionTiers[Math.floor(Math.random() * (contributionTiers.length - 1))],
      durations[0],
      0,
      durations,
    );
