...
export function processCharacters(): { [key: string]: any } {
    ...
    for (const id of Object.keys(characters)) {
        ...
        for (const l of Object.keys(characters[id]['layers'])) {
            ...
            traits = traits.add(`0x${(trait.indexOf(feature) + 1).toString(16)}${('0').repeat(traitsShiftOffset[l] / 4)}`);
        }
 ...
 */
export async function loadAsset(storage: any, signer: SignerWithAddress, asset: string, assetId: string) {
    let assetParts: any;
    let inflatedSize = 0;
