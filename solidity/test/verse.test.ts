...
describe('BannyVerse Tests', () => {
  ...
  before(async () => {
    merkleSnapshot = MerkleHelper.makeSampleSnapshot(
      accounts.filter((a, i) => i >= merkleAddressOffset).map((a) => a.address),
    );
    ...
    const tokenFactory = await ethers.getContractFactory('Token');
    merkleToken = await tokenFactory
      .connect(deployer)
      .deploy(
        storage.address,
        bannyCommonUtilLibrary.address,
  ...
  it('Basic Mint Tests', async () => {
    ...
    await expect(token.connect(deployer).mint(accounts[0].address, tokenTraits))
    ...
    const decoded = Buffer.from(
      dataUri.slice('data:application/json;base64,'.length),
      'base64',
    ).toString();
    ...
    const imageData = Buffer.from(
      decodedJson['image'].slice(decodedJson['image'].indexOf(',') + 1),
      'base64',
    ).toString();
  ...
  it('Contract Metadata Tests', async () => {
    await expect(token.connect(deployer).setContractURI('')).to.be.revertedWith(
      'ARGUMENT_EMPTY("_uri")',
    );
  ...
  it('Transfer Tests', async () => {
    ...
    await token
      .connect(accounts[1])
      ['safeTransferFrom(address,address,uint256)'](accounts[1].address, accounts[0].address, 1);
  ...
  it('Merkle Mint Tests', async () => {
    ...
    for (let i = 0; i < merkleItem.data - 1; i++) {
      ...
      await expect(
        merkleToken
          .connect(merkleAccount)
          .merkleMint(merkleItem.index, merkleItem.data, merkleItem.proof, basicBananaTraits),
      )
        .to.emit(merkleToken, 'Transfer')
        .withArgs(ethers.constants.AddressZero, merkleAccount.address, tokenId);
    ...
    const decoded = Buffer.from(
      tokenData.slice('data:application/json;base64,'.length),
      'base64',
    ).toString();
    expect(
      JSON.parse(decoded)['animation_url'].slice('data:audio/mp3;base64,'.length).length,
    ).to.equal(160836);
